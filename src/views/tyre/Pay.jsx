...
import { Button, Toast, Modal, List, Icon, Flex, WhiteSpace, Carousel, Stepper, Steps } from 'antd-mobile';
...
export default class Pay extends React.Component {
    ...
    componentDidMount () {
        const parts = this.state.selectedItem.partses[0].parts;
        const imgs = [];
        if(parts.partsCoverImage && parts.partsCoverImage.attachmentId) {
            imgs.push(parts.partsCoverImage);
        }
        if(parts.partsAttached1Image && parts.partsAttached1Image.attachmentId) {
            imgs.push(parts.partsAttached1Image);
        }
        if(parts.partsAttached2Image && parts.partsAttached2Image.attachmentId) {
            imgs.push(parts.partsAttached2Image);
        }
        if(parts.partsAttached3Image && parts.partsAttached3Image.attachmentId) {
            imgs.push(parts.partsAttached3Image);
        }
        if(parts.partsAttached4Image && parts.partsAttached4Image.attachmentId) {
            imgs.push(parts.partsAttached4Image);
        }
        this.setState({
            imgs
        })
                        ...
                        <List.Item>
                            ...
                            >
                                {imgs.map(img=>(
                                    <a
                                        key={img.id}
                                        style={{ display: 'inline-block', width: '100%', textAlign: 'center', backgroundColor: '#fff' }}
                                    >
                                        <img
                                            src={`${util.baseURL}/attachment/download/${img.attachmentId}`}
                                            alt=""
                                            style={{ height: 250, width: 'auto', verticalAlign: 'top' }}
                                        />
                                    </a>
                                ))}
                            </Carousel>
                        </List.Item>
                        <List.Item extra={<span style={{color: '#C3A769'}}>
                                ￥ {numeral(selectedItem.partses[0].parts.price).format('0,0.0')}
                            </span>}>
                            {selectedItem.partses[0].parts.name}
                        </List.Item>
                        <List.Item extra={<Stepper
                            showNumber
                            value={selectedItem.partses[0].count}
                            min={1}
                            max={4}
                            onChange={(v)=>{
                                selectedItem.partses[0].count = v;
                                this.setState({
                                    selectedItem
                                })
                            }}></Stepper>}>
